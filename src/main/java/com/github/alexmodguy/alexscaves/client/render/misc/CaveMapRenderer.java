...
public class CaveMapRenderer {
    ...
    private void updateTexture() {
        Registry<Biome> registry = Minecraft.getInstance().level.registryAccess().registry(Registry.BIOME_REGISTRY	).orElse(null);
        if (registry != null && mapBiomes.length >= 128 * 128) {
            for (int i = 0; i < 128; ++i) {
                for (int j = 0; j < 128; ++j) {
                    ...
                    int r = red(biomeColor);
                    int g = green(biomeColor);
                    int b = blue(biomeColor);
                    double edge = Math.sqrt((i - 64) * (i - 64) + (j - 64) * (j - 64)) / 128;
                    int alpha = transparent ? Math.max(255 - (int) (255F * (edge * edge)), 10) : 255;
                    this.texture.getPixels().setPixelRGBA(j, i, color(alpha, b, g, r));
                }
            }
        }
        this.texture.upload();
    ...
    public void renderLabels(PoseStack poseStack, MultiBufferSource multiBufferSource, int light) {
        for (BiomeLabel label : labels) {
            ...
            poseStack.mulPose(Vector3f.ZP.rotationDegrees(label.rotation()));
            poseStack.pushPose();
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, ItemStack map, boolean fullFrame, int light) {
        ...
        if (!fullFrame) {
            ...
            poseStack.pushPose();
            poseStack.mulPose(Vector3f.ZP.rotationDegrees((float) Minecraft.getInstance().player.getYRot() + 180));
