...
public class SubmarineRenderer extends EntityRenderer<SubmarineEntity> {
    ...
    public static void renderSubmarine(SubmarineEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource source, int lightIn, boolean maskWater) {
        ...
        poseStack.mulPose(Vector3f.YP.rotationDegrees(180 - submarineYaw));
        poseStack.scale(-1.0F, -1.0F, 1.0F);
        poseStack.mulPose(Vector3f.XN.rotationDegrees(submarinePitch));
        poseStack.translate(0, 0, 0);
        if (entity.getWaterHeight() > 0 && entity.getWaterHeight() < 1.6F) {
            poseStack.mulPose(Vector3f.ZP.rotationDegrees((float) (Math.sin(ageInTicks * 0.1F) * 0.5F)));
            poseStack.mulPose(Vector3f.XP.rotationDegrees((float) (Math.sin(ageInTicks * 0.1F + 1.3F) * 0.5F)));
        }
        poseStack.pushPose();
        ...
        for (Entity passenger : entity.getPassengers()) {
            ...
            poseStack.mulPose(Vector3f.YN.rotationDegrees(360 - submarineYaw));
        ...
        if (!isFirstPersonFloodlightsMode(entity) && entity.areLightsOn() && entity.isVehicle()) {
            ...
            poseStack.mulPose(Vector3f.YN.rotationDegrees(submarineYaw));
            poseStack.mulPose(Vector3f.YP.rotationDegrees(yRot));
            poseStack.mulPose(Vector3f.XN.rotationDegrees(90 - xRot));
