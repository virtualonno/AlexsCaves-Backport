...
public class ACAdvancementTabs {
    ...
    public static void renderTabBackground(PoseStack guiGraphics, int topX, int topY, DisplayInfo displayInfo, double scrollX, double scrollY) {
        float partialTick = Minecraft.getInstance().getPartialTick();
        float hoverProgress = getHoverChangeAmount(partialTick);
        float priorHoverProgress = 1F - hoverProgress;
        int fastColor = lerp(hoverProgress, previousHoverType.backgroundColor, hoverType.backgroundColor);
        Screen.fill(guiGraphics, 0, 0, windowWidth + 100, windowHeight, fastColor | -16777216);
    ...
    public static int lerp(float p_270972_, int p_270081_, int p_270150_) {
    	int i = lerpInt(p_270972_, FastColor.ARGB32.alpha(p_270081_), FastColor.ARGB32.alpha(p_270150_));
        int j = lerpInt(p_270972_, FastColor.ARGB32.red(p_270081_), FastColor.ARGB32.red(p_270150_));
        int k = lerpInt(p_270972_, FastColor.ARGB32.green(p_270081_), FastColor.ARGB32.green(p_270150_));
        int l = lerpInt(p_270972_, FastColor.ARGB32.blue(p_270081_), FastColor.ARGB32.blue(p_270150_));
        return FastColor.ARGB32.color(i, j, k, l);
    }
    
    public static int lerpInt(float p_270245_, int p_270597_, int p_270301_) {
    	return p_270597_ + Mth.floor(p_270245_ * (float)(p_270301_ - p_270597_));
    }

    private static void renderTabBackgroundForType(PoseStack guiGraphics, int topX, int topY, float partialTick, double scrollX, double scrollY, Type type, float alpha) {
