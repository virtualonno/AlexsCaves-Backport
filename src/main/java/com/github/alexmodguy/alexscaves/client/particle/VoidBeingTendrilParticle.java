...
public class VoidBeingTendrilParticle extends Particle {
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        cameraOffset.transform(quaternion);
    ...
    private Quaternion calcCameraAngle(Camera camera, float sampleScale) {
        float followCameraAmount = sampleScale * sampleScale;
        Quaternion quater = new Quaternion(camera.rotation());
		quater.mul(Vector3f.XP.rotation(followCameraAmount * -0.017453292F * camera.getXRot()));
		quater.mul(Vector3f.YP.rotation(followCameraAmount * 0.017453292F * camera.getYRot()));
        return quater;
    }
