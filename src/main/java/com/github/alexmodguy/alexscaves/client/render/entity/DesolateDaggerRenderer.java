...
public class DesolateDaggerRenderer extends EntityRenderer<DesolateDaggerEntity> {
    ...
    public void render(DesolateDaggerEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource source, int lightIn) {
        ...
        poseStack.mulPose(Vector3f.YN.rotationDegrees(Mth.lerp(partialTicks, entity.yRotO, entity.getYRot()) + 90.0F));
        poseStack.mulPose(Vector3f.ZN.rotationDegrees((float) (Mth.lerp(partialTicks, entity.xRotO, entity.getXRot()) + 5F * Math.sin(ageInTicks * 0.2F))));
        poseStack.mulPose(Vector3f.ZN.rotationDegrees(45));
        poseStack.translate(-0.5D, -0.5D, -0.5D);
        BakedModel bakedmodel = Minecraft.getInstance().getItemRenderer().getModel(entity.daggerStack, entity.level, null, 0);
        float f = 1.0F;
        float f1 = 0;
        float f2 = 0;
        ...
        for (net.minecraft.client.renderer.RenderType rt : bakedmodel.getRenderTypes(entity.daggerStack, false)) {
