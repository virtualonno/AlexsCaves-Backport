...
public class PageRenderer {
    ...
    protected void renderPage(CaveBookScreen screen, PoseStack poseStack, int mouseX, int mouseY, float partialTicks, boolean onFlippingPage) {
        ...
        if(entry != null){
            ...
            screen.getMinecraft().font.drawInBatch(String.valueOf(pgNumber), 86.0F - numberWidth * 0.5F, 210.0F, CaveBookScreen.TEXT_COLOR, false, poseStack.last().pose(), bufferSource, false, 0, 15728880);
            poseStack.popPose();
        ...
        if (startReadingAt >= 0) {
            for (int i = startReadingAt; i < startReadingAt + CaveBookScreen.PAGE_SIZE_IN_LINES; i++) {
                if (bookEntry.getEntryText().size() > i) {
                    ...
                    font.drawInBatch(printLine, 0.0F, (i - startReadingAt) * 10, CaveBookScreen.TEXT_COLOR, false, poseStack.last().pose(), bufferSource, false, 0, 15728880);
                    for (BookLink bookLink : bookEntry.getEntryLinks()) {
                        if (bookLink.getLineNumber() == i) {
                            ...
                            font.drawInBatch(component, fontWidth, (i - startReadingAt) * 10, bookLink.isEnabled() ? bookLink.isHovered() ? CaveBookScreen.TEXT_LINK_HOVER_COLOR : CaveBookScreen.TEXT_LINK_COLOR : CaveBookScreen.TEXT_LINK_LOCKED_COLOR, false, poseStack.last().pose(), bufferSource, false, 0, 15728880);
