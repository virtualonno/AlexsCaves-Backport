...
public class RaygunRenderHelper {
    ...
    private static void renderRay(PoseStack poseStack, MultiBufferSource bufferSource, Vec3 vec3, float useAmount, float offset, boolean irradiated) {
        ...
        poseStack.mulPose(Vector3f.YP.rotationDegrees(((Mth.PI / 2F) - f6) * Mth.RAD_TO_DEG));
        poseStack.mulPose(Vector3f.XP.rotationDegrees(f5 * Mth.RAD_TO_DEG));
        poseStack.mulPose(Vector3f.YP.rotationDegrees(offset * 3.0F));
        float f8 = 1F;
        int j = (int) (f8 * 255.0F);
        int k = (int) (f8 * 255.0F);
        int l = (int) (f8 * 255.0F);
