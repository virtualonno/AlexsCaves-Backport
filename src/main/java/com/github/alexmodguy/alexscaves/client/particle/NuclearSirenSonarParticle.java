...
public class NuclearSirenSonarParticle extends TextureSheetParticle {
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        this.renderSignal(vertexConsumer, camera, partialTick, (quaternionf) -> {
            quaternionf.mul(Vector3f.YP.rotation(-(float) Math.toRadians(yRot)));
            quaternionf.mul(Vector3f.XP.rotation(-(float) Math.toRadians(xRot)));
        });
        this.renderSignal(vertexConsumer, camera, partialTick, (quaternionf) -> {
            quaternionf.mul(Vector3f.YP.rotation(-(float) Math.PI - (float) Math.toRadians(yRot)));
            quaternionf.mul(Vector3f.XP.rotation((float) Math.toRadians(xRot)));
        });
    }
