...
public class RaygunBlastParticle extends TextureSheetParticle {
    ...
    public void tick() {
        ...
        this.yd = 0;
        this.zd = 0;
        BlockPos connectedTo = new BlockPos(this.x + direction.getStepX() * -0.1F, this.y + direction.getStepY() * -0.1F, this.z + direction.getStepZ() * -0.1F);
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        this.renderSignal(vertexConsumer, camera, partialTick, (quaternionf) -> {
            ...
            quaternionf.mul(Vector3f.XP.rotation(-(float) Math.PI * 0.5F));
        });
        this.renderSignal(vertexConsumer, camera, partialTick, (quaternionf) -> {
            quaternionf.mul(direction.getRotation());
            quaternionf.mul(Vector3f.XP.rotation((float) Math.PI - (float) Math.PI * 0.5F));
        });
    }

    private void renderSignal(VertexConsumer consumer, Camera camera, float partialTicks, Consumer<Quaternion> rots) {
        ...
        vector3f.normalize();
        Quaternion quaternionf = new Quaternion(new Vector3f(vector3f.x(), vector3f.y(), vector3f.z()), 0.0F, false);
        rots.accept(quaternionf);
        quaternionf.mul(Vector3f.ZP.rotation(randomRot));
