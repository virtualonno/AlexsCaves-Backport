...
public class WatcherRenderer extends MobRenderer<WatcherEntity, WatcherModel> {
    ...
    public void render(WatcherEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        ...
        if (!shouldSit && entity.isAlive()) {
            f8 = this.speed(entity, partialTicks);
            f5 = this.position(entity, partialTicks);
            if (entity.isBaby()) {
                f5 *= 3.0F;
            }
    ...
    public float speed(LivingEntity entity, float p_268054_) {
        return Mth.lerp(p_268054_, entity.animationSpeedOld, entity.animationSpeed);
    }

    public float position(LivingEntity entity, float p_268007_) {
    	return entity.animationPosition - entity.animationSpeed * (1.0F - p_268007_);
    }
