...
public class GuanoPileFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        while (pileBottom.getY() > level.getMinBuildHeight() && level.getBlockState(pileBottom).getMaterial().isReplaceable()) {
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                ...
                while (!level.getBlockState(side).getMaterial().isReplaceable() && side.getY() < level.getMaxBuildHeight()) {
                    side.move(0, 1, 0);
                }
                while (level.getBlockState(side).getMaterial().isReplaceable() && side.getY() > level.getMinBuildHeight()) {
                    side.move(0, -1, 0);
                }
