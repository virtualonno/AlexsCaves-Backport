...
public class MagnetronEntity extends Monster {
    ...
    public void tick() {
        ...
        if (speed > 0.01) {
            if (this.isFormed()) {
                float f = (float) Math.cos(this.animationPosition * 0.4F - 1.5F);
                if (this.animationSpeed > 0.2F && Math.abs(f) < 0.2F) {
    ...
    public void spawnGroundEffects() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int i1 = 0; i1 < 20 + random.nextInt(12); i1++) {
                ...
                BlockPos ground = new BlockPos(ACMath.getGroundBelowPosition(level, new Vec3(Mth.floor(center.x + extraX), Mth.floor(center.y + extraY) - 1, Mth.floor(center.z + extraZ))));
