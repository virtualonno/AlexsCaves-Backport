        ...
        if (!level.isClientSide) {
            ...
            if (entity.spawnsMobIn <= 0) {
                if (entity.spawnType == null) {
                    ...
                    if (entity.spawnMobs()) {
                        level.playSound((Player)null, blockPos, ACSoundRegistry.AMBER_MONOLITH_SUMMON.get(), SoundSource.BLOCKS, 1.0F, 1.0F);
                        entity.generateSpawnData();
                    }
                }
            }
        ...
        for (int l1 = 0; l1 < spawnCount; l1++) {
            ...
            for (int i2 = 0; !flag && i2 < 6; i2++) {
                ...
                if (spawnType.canSummon() && NaturalSpawner.isSpawnPositionOk(SpawnPlacements.getPlacementType(spawnType), level, blockpos, spawnType)) {
                    ...
                    if (!level.noCollision(spawnType.getAABB(d0, (double) blockpos.getY(), d1)) || !SpawnPlacements.checkSpawnRules(spawnType, (ServerLevelAccessor) level, MobSpawnType.CHUNK_GENERATION, new BlockPos(d0, (double) blockpos.getY(), d1), level.getRandom())) {
                        continue;
                    }
