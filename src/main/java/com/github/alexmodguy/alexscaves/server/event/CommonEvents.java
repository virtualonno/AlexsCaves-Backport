...
public class CommonEvents {
    ...
    public void livingHurt(LivingDamageEvent event) {
        if (event.getEntity().isPassenger() && event.getEntity() instanceof FlyingMount && (event.getSource().equals(DamageSource.IN_WALL) || event.getSource().isFall() || event.getSource().equals(DamageSource.FLY_INTO_WALL))) {
            event.setCanceled(true);
        }
        if (event.getEntity() instanceof WatcherPossessionAccessor possessed && possessed.isPossessedByWatcher() && !event.getSource().isBypassInvul() && !(event.getSource().getEntity() instanceof WatcherEntity)) {
            event.setCanceled(true);
        }
    }
