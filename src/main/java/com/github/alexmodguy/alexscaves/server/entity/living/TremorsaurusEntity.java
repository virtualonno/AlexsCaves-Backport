...
public class TremorsaurusEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen {
    ...
    public void tick() {
        ...
        if (this.isOnGround() && !this.isInFluidType() && this.animationSpeed > 0.1F && !this.isBaby()) {
            float f = (float) Math.cos(this.animationPosition * 0.8F - 1.5F);
            if (Math.abs(f) < 0.2) {
        ...
        if (!level.isClientSide) {
            ...
            if (riderHitEntity != null && this.getAnimation() == ANIMATION_BITE && this.getAnimationTick() > 10 && this.getAnimationTick() <= 12) {
                if (this.hasLineOfSight(riderHitEntity) && this.distanceTo(riderHitEntity) < this.getBbWidth() + riderHitEntity.getBbWidth() + 2.0D) {
                    riderHitEntity.hurt(DamageSource.mobAttack(this), (float) this.getAttribute(Attributes.ATTACK_DAMAGE).getValue());
    ...
    public void aiStep() {
		...
		if (livingentity instanceof Player player) {
			if (this.isAlive()) {
				this.travelRidden(player, new Vec3((double)this.xxa, (double)this.yya, (double)this.zza));
			}
		}
    }
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger) && passenger instanceof LivingEntity living && !this.touchingUnloadedChunk()) {
            Vec3 seatOffset = new Vec3(0F, 1F, 0.6F).yRot((float) Math.toRadians(-this.yBodyRot));
        ...
        if (keyPresser.isPassengerOfSameVehicle(this)) {
            ...
            if (type == 3) {
                if (this.getAnimation() == NO_ANIMATION || this.getAnimation() == null) {
                    HitResult hitresult = getHitResultOnViewVector(keyPresser, entity -> !entity.is(this) && !this.isAlliedTo(entity), 10.0F);
