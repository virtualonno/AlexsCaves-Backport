...
public class CorrodentEntity extends Monster implements ICustomCollisions, IAnimatedEntity {
    ...
    public void tick() {
        ...
        if (isDigging() && surfacePosition != null) {
            if (level.isClientSide && isMoving()) {
                BlockState surfaceState = this.level.getBlockState(new BlockPos(surfacePosition).below());
                BlockState onState = this.getFeetBlockState();
                if (surfaceState.getMaterial().isSolid()) {
                ...
                if (onState.getMaterial().isSolid()) {
    ...
    public void handleEntityEvent(byte b) {
        if (b == 77) {
            ...
            for (int i1 = 0; i1 < particleCount; i1++) {
                ...
                BlockPos ground = new BlockPos(ACMath.getGroundBelowPosition(level, new Vec3(Mth.floor(this.getX() + extraX), Mth.floor(this.getY() + extraY) + 2, Mth.floor(this.getZ() + extraZ))));
                BlockState groundState = this.level.getBlockState(ground);
                if (groundState.getMaterial().isSolid()) {
                    if (level.isClientSide) {
    ...
    public int getCorrosionAmount(BlockPos pos) {
        ...
        if (distance <= 10) {
            ...
            if (canDigBlock(state) && !state.isAir() && !state.getMaterial().isReplaceable()) {
    ...
    public boolean isInvulnerableTo(DamageSource damageSource) {
        return super.isInvulnerableTo(damageSource) || damageSource.equals(DamageSource.IN_WALL);
    }
    ...
    protected void checkFallDamage(double y, boolean isOnGroundIn, BlockState state, BlockPos pos) {
        if (!isDigging()) {
            super.checkFallDamage(y, isOnGroundIn, state, pos);
        }
    }
    ...
    class Navigator extends FlyingPathNavigation {
        ...
        public boolean canCutCorner(BlockPathTypes p_265292_) {
        	return p_265292_ != BlockPathTypes.DANGER_FIRE && p_265292_ != BlockPathTypes.DANGER_OTHER && p_265292_ != BlockPathTypes.WALKABLE_DOOR;
        }
    ...
    private class DiggingMoveControl extends MoveControl {
        ...
        public void tick() {
            if (this.operation == MoveControl.Operation.MOVE_TO) {
                Vec3 ed = Vec3.atCenterOf(this.mob.getNavigation().getTargetPos());
                ...
                if (isSafeDig(level, new BlockPos(wantedX, wantedY, wantedZ))) {
