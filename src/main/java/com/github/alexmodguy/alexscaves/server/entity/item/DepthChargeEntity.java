...
public class DepthChargeEntity extends ThrowableItemProjectile {
    ...
    public void tick(){
        ...
        if(this.isOnGround() || this.horizontalCollision || this.verticalCollision){
            hitGround = true;
        }
        if (this.isInWater()) {
            for(int i = 0; i < 4; ++i) {
    ...
    private void explode() {
        ...
        Explosion.BlockInteraction blockinteraction = net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(level, this) ? level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) ? Explosion.BlockInteraction.BREAK : Explosion.BlockInteraction.DESTROY : Explosion.BlockInteraction.NONE;
