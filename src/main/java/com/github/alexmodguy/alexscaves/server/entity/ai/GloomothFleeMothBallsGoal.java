...
public class GloomothFleeMothBallsGoal extends Goal {
    ...
    public boolean canContinueToUse() {
        return blockPos != null && this.gloomoth.distanceToSqr(Vec3.atCenterOf(blockPos)) < 32;
    }
    ...
    public void tick() {
        ...
        if(retreatTo == null || gloomoth.distanceToSqr(retreatTo) < 4){
            for (int i = 0; i < 15; i++) {
                Vec3 vec3 = DefaultRandomPos.getPosAway(gloomoth, 32, 15, Vec3.atCenterOf(blockPos));
                if (vec3 != null) {
                    retreatTo = vec3;
                    break;
                }
