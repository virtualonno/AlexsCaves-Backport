...
public class SubterranodonFlightGoal extends Goal {
    ...
    private Vec3 findFlightPos() {
        ...
        if(!entity.level.isLoaded(new BlockPos(targetVec))){
        ...
        if (entity.level.canSeeSky(new BlockPos(heightAdjusted))) {
            ...
            while (ceiling.getY() < entity.level.getMaxBuildHeight() && !entity.level.getBlockState(ceiling).getMaterial().isSolid()) {
    ...
    public Vec3 groundPosition(Vec3 airPosition) {
        ...
        boolean flag = false;
        while (ground.getY() < entity.level.getMaxBuildHeight() && !entity.level.getBlockState(ground).getMaterial().isSolid() && entity.level.getFluidState(ground).isEmpty()){
            ground.move(0, 1, 0);
            flag = true;
        }
        ground.move(0, -1, 0);
        while (ground.getY() > entity.level.getMinBuildHeight() && !entity.level.getBlockState(ground).getMaterial().isSolid() && entity.level.getFluidState(ground).isEmpty()) {
            ground.move(0, -1, 0);
        }
