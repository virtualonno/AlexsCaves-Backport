        ...
        if (head instanceof BoundroidEntity boundroid) {
            if (!level.isClientSide) {
                ...
                if (this.isLatched()) {
                    ...
                    } else if (this.isOnGround() && noLatchCooldown == 0 && this.isAlive() && random.nextInt(30) == 0 && distanceToCeiling > MAX_DIST_TO_CEILING && hasBlockAbove()) {
                        goingUp = true;
                    }
                }
    ...
    private boolean hasBlockAbove() {
        if(!this.level.canSeeSky(this.blockPosition())){
            ...
            while (mutableBlockPos.getY() < this.level.getMaxBuildHeight()){
                if(this.level.getBlockState(mutableBlockPos).getMaterial().isSolid()){
                    return true;
                }
