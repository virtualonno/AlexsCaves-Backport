...
public class HullbreakerEntity extends WaterAnimal implements IAnimatedEntity {
    ...
    private void populateDeathLootForHullbreaker(){
        ...
        if(damageSource != null){
            ...
            LootContext.Builder lootparams$builder = (new LootContext.Builder((ServerLevel)this.level)).withParameter(LootContextParams.THIS_ENTITY, this).withParameter(LootContextParams.ORIGIN, this.position()).withParameter(LootContextParams.DAMAGE_SOURCE, damageSource).withOptionalParameter(LootContextParams.KILLER_ENTITY, damageSource.getEntity()).withOptionalParameter(LootContextParams.DIRECT_KILLER_ENTITY, damageSource.getDirectEntity());
            ...
            LootContext lootparams = lootparams$builder.create(LootContextParamSets.ENTITY);
            loottable.getRandomItems(lootparams).forEach(deathItems::add);;
        }
    }
    ...
    private double swimDegree(float width, float sinOffset) {
        double move = Math.cos(this.animationPosition * 0.33F + sinOffset) * this.animationSpeed * width * 0.8F;
        double idle = Math.sin((tickCount + AlexsCaves.PROXY.getPartialTicks()) * 0.05F + sinOffset) * width * 0.5F;
        return (move + idle * (1 - this.animationSpeed)) * (1 - getLandProgress(AlexsCaves.PROXY.getPartialTicks()));
    }
